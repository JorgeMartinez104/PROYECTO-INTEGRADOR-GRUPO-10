...
const pErrorContact = formContact.getElementsByClassName('formInputContact-error');
...
formContact.addEventListener("submit", (event) => {
  ...
  if (condition) {
    formContact.reset();
    document.getElementById('formMessContact-exito').classList.add('formMessContact-active');
		setTimeout(() => {
			document.getElementById('formMessContact-exito').classList.remove('formMessContact-active');
		}, 6000);
    ...
    document.querySelectorAll('.groupFormContact-ok').forEach((icono) => {
		...
		document.querySelectorAll('.inputError-inactive').forEach((inputError) => {
  ...
  switch (attName) {
    case "nombre":
      ...
      if(nameForm.value.trim() == "")
      {
        ...
        insertTextLastElement(`#group${fieldsGroup[attName]}`, "*Nombre no válido.");
        condi = false;
      }
      else{
    ...
    case "apellido":
      ...
      {
        validateField(fieldsGroup[attName], false);
        insertTextLastElement(`#group${fieldsGroup[attName]}`, "*Apellido no válido.");
        condi = false;
      }
      else
      {
    ...
    case "telefono":
      ...
      {
        validateField(fieldsGroup[attName], false);
        insertTextLastElement(`#group${fieldsGroup[attName]}`, "*Teléfono no válido.");
        condi = false;
      }
      else
      {
    ...
    case "mail":
      ...
      {
        validateField(fieldsGroup[attName], false);
        insertTextLastElement(`#group${fieldsGroup[attName]}`, "*Email no válido.");
        condi = false;
      }
      else
      {
      ...
      case "fecha_fin":
        ...
        {
          validateField(fieldsGroup[attName], false);
          insertTextLastElement(`#group${fieldsGroup[attName]}`, "*Debe ingresar un valor en campo fecha.");
          condi = false;
        }
        ...
        {
          validateField(fieldsGroup[attName], false);
          insertTextLastElement(`#group${fieldsGroup[attName]}`, "*Fecha hasta no puede ser menor que Fecha desde.");
          condi = false;
        }
        if(condi)
        {
...
const validateField = (field, condition) => {
	if(condition){
		document.getElementById(`group${field}`).classList.remove('groupFormContact-wrong');
		document.getElementById(`group${field}`).classList.add('groupFormContact-ok');
    ...
    matches.forEach(function (ele) {
      ...
      ele.classList.remove('inputError-active');
		...
		document.querySelector(`#group${field} .formInputContact-error`).classList.remove('formInputContact-error-active');
	} else {
		document.getElementById(`group${field}`).classList.add('groupFormContact-wrong');
		document.getElementById(`group${field}`).classList.remove('groupFormContact-ok');
    var container = document.querySelector(`#group${field}`);
    var matches = container.querySelectorAll('input');
    matches.forEach(function (ele) {
    ...
    ele.classList.remove('inputError-inactive');
		...
		document.querySelector(`#group${field} .formInputContact-error`).classList.add('formInputContact-error-active');
	}
}
...
const reset = (e) => {
  ...
  inputsForm.forEach((element) =>
  {
    ...
    switch (attrName) {
      ...
      case "fecha_inicio":
      case "fecha_fin":
        var container = document.querySelector(`#group${fieldsGroup[attrName]}`);
        var matches = container.querySelectorAll('input');
        matches.forEach(function (ele) {
          ele.classList.remove('inputError-active');
  ...
  for (let inputCont of inputsCont)
  inputCont.value = ''
}
